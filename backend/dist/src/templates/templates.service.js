...
const docx_importer_1 = require("./utils/docx-importer");
...
let TemplatesService = class TemplatesService {
    ...
    generateUniqueTemplateName(requestedName, currentTemplateId) {
        const raw = requestedName.trim() || 'Untitled Template';
        const existingNames = new Set(Array.from(this.templates.values())
    ...
    async createFromDocx(originalName, docxBuffer) {
        ...
        const imported = await (0, docx_importer_1.parseDocxFull)(docxBuffer);
        const braceCheck = (0, placeholder_parser_1.validateBraces)(imported.bodyHtml);
        if (!braceCheck.valid) {
    ...
    async createFromStarter(source, templateName, options) {
        ...
        const record = {
            ...
            marginLeft: DEFAULT_LEFT,
            marginRight: DEFAULT_RIGHT,
            placeholders: (0, placeholder_parser_1.extractPlaceholdersFromDocx)(docxBuffer),
            source: source,
            createdAt: now,
            updatedAt: now,
        };
